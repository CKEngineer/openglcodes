...
GLfloat vertices[] =
{
	-0.8f,-0.8f, /*Lower left corner */ 
	 0.8f,-0.8f, /* Lower right corner */
	 0.0f, 0.8f, /* Upper right corner */
};
...
int main()
{
	...
	VBO1.LinkAttrib(VBO1,aPosition,2,GL_FLOAT,2 * sizeof(float), (void*)0); //GL_FALSE 5(arrayin 1 satırının uzunluğu kadr) * sizeof(float) // Ve nereden başladığı (void *)
    CHECK_GL_ERROR;
	...
	shaderProgram.RetriveUniformf("u_time",&monitor_param);
	std::cout<<monitor_param<<"\n"<<std::endl;
	time += 0.01f;
        if (time > 2 * M_PI) {
            time = 0.0f;
        }
	...
	shaderProgram.QueryUniformVariables();
