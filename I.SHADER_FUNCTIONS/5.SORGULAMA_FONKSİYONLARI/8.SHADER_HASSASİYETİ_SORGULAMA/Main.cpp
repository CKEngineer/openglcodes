...
GLfloat vertices[] =
{
	-0.7f, -0.7f, /*Lower left corner */ 0.0f,0.0f,1.0f,
	0.7f, -0.7f, /* Lower right corner */1.0f, 1.0f, 1.0f,
	0.7f, 0.7f, /* Upper right corner */ 0.0f,1.0f,0.0f,
	-0.7f, 0.7f, /* Upper left corner */ 1.0f,0.0f,0.0f
};
...
int main()
{
	...
	shaderProgram.SetUniform2f(SCR_WIDTH,SCR_HEIGHT,"u_resolution");
	shaderProgram.SetUniform1f(time,"u_time");
	time += 0.01f;
    if (time > 2 * M_PI) {
            time = 0.0f;
        }
    ...
    shaderProgram.QueryShaderPrecision(GL_LOW_INT);
	shaderProgram.QueryShaderPrecision(GL_MEDIUM_INT);
	shaderProgram.QueryShaderPrecision(GL_HIGH_INT);

    shaderProgram.QueryShaderPrecision(GL_LOW_FLOAT);
	shaderProgram.QueryShaderPrecision(GL_MEDIUM_FLOAT);
	shaderProgram.QueryShaderPrecision(GL_HIGH_FLOAT);
    shaderProgram.QueryShaderPrecision(GL_VERTEX_SHADER);
	shaderProgram.QueryShaderPrecision(GL_FRAGMENT_SHADER);
