...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "beginner2_glTexSubImage2D", NULL, NULL);
	...
	Texture pixel_128x128(NULL,4,4,GL_TEXTURE_2D, GL_TEXTURE0, GL_RGB, GL_UNSIGNED_BYTE);
	CHECK_GL_ERROR;
	pixel_128x128.texUnit(shaderProgram, "tex0", 0);
	CHECK_GL_ERROR;
	glBindTexture(GL_TEXTURE_2D,pixel_128x128.ID);
	glTexSubImage2D(GL_TEXTURE_2D,0           ,0,0,2,2,GL_RGB,GL_UNSIGNED_BYTE,update);
	glTexSubImage2D(GL_TEXTURE_2D,0           ,2,0,2,2,GL_RGB,GL_UNSIGNED_BYTE,update2);
	glTexSubImage2D(GL_TEXTURE_2D,0           ,0,2,2,2,GL_RGB,GL_UNSIGNED_BYTE,update3);
    glTexSubImage2D(GL_TEXTURE_2D,0           ,2,2,2,2,GL_RGB,GL_UNSIGNED_BYTE,update4);
    CHECK_GL_ERROR;
